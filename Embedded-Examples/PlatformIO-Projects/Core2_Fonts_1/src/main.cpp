...
/*
  ...
  i.e. GLCD (Font 1), Font 2, Font 4, Font 6, Font 7, Font 8 and five Free
...
void loop() {
    ...
    // For comaptibility with Adafruit_GFX library the text background is not
    // plotted when using the print class even if we specify it.
    ...
    M5.Lcd.print("The really tiny TomThumb font");  // Print the font name onto
                                                    // the TFT screen
    M5.Lcd.println();
    M5.Lcd.println();
    ...
    M5.Lcd.println();  // Free fonts plot with the baseline (imaginary line the
                       // letter A would sit on)
    // as the datum, so we must move the cursor down a line from the 0,0
    // position
    ...
    // Set text padding to 100 pixels wide area to over-write old values on
    // screen
